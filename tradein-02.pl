#!/usr/bin/perl
## Filter caps by lengh and process with tradein approach (two points traded).
...
sub tradein {
	...
	# Initiate array with cap elements 
	my @array = @$cap;
	my $s = @array;

	# Drop two last points
	splice @array, $s-2, 2;

	# Create the array with indexes
	my @seq = 0..$#array;

	# Create two-cards combinations from indexes
	my @cmbs = combinations(\@seq, 2);
